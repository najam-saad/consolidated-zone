...
export function Services() {
  ...
  return (
    <section className="py-20 relative scanline-container" id="services">
      {/* Scanline effect */}
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl xl:text-5xl font-bold mb-6 gradient-text">
            Our Services
          </h2>
          <p className="max-w-2xl mx-auto text-gray-400 typing-text">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {services.map((service, index) => (
            <div
              ...
              className={`service-card bg-black/30 backdrop-blur-sm rounded-xl overflow-hidden transition-all duration-500 border border-violet-900/30 hover:border-violet-700/50 group hover-card card-3d stagger-item ${visibleItems.includes(`service-${service.id}`) ? 'stagger-item-visible' : ''}`}
              style={{ animationDelay: `${index * 0.1}s` }}
            >              
              <div className="p-6">
                <div className="w-12 h-12 rounded-full bg-black/50 flex items-center justify-center text-white mb-4 border border-violet-700/30 group-hover:text-secondary group-hover:border-secondary transition-colors glow-element">
                  {service.icon}
                </div>
        ...
        <div className="text-center">
          <Link
            href="/services"
            className="btn-cyberpunk inline-flex items-center justify-center px-6 py-3 rounded-full text-white font-medium ripple"
          >
            View All Services
